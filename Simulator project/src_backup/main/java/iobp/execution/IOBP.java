...
public class IOBP {
    ...
    HashMap<String, BigInteger> startTasks;
    int n;
    //Deploy triggers
    AtomicInteger eventsInCurrentTraceWithNoise;
    ...
    public void setup(ArrayList<String> functionsCalls, HashMap<String, String> orgsAddresess, String supervisorPrivateKey, int nInstances, int noisePercentage, int traceAverageSize, boolean performDataCleaning) throws Exception {
        ...
        this.n = nInstances;
        for (Map.Entry<String, BigInteger> entry : startTasks.entrySet()) {
            for(Organization o : collaborators){
                if(o.CREDENTIALS.getAddress().equals(entry.getKey())){
                    for(int i = 0; i < this.n; i++)
                        o.executeTask(entry.getValue(), new BigInteger(String.valueOf(i+1), 10));
                }
            }
        }
